...
// Icon mapper helper
const getCategoryIcon = (iconName?: string, type?: 'food' | 'drinks') => {
  switch (iconName?.toLowerCase()) {
    ...
    case 'juices':
      return Citrus;
    default:
      return type === 'food' ? Utensils : CupSoda;
  }
};

export default function CategoryList({ category, onBack }: CategoryListProps) {
  const { products, addOrder, restaurants, stores, categories } = useApp();
  ...
  const displayCategories = categories.filter((c) => c.type === category && c.active !== false);

  const currentCategoryObj = displayCategories.find((i) => i.id === selectedSubCat);
    ...
    <div className="w-full max-w-5xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300 font-cairo">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white/90 backdrop-blur-md p-6 rounded-2xl border border-stone-200/90 shadow-sm text-right">
        ...
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-50 text-emerald-700 text-xs font-semibold border border-emerald-200/60 self-end sm:self-center">
          ...
          <span>{selectedSubCat ? `${categoryProducts.length} منتج متوفر` : `${displayCategories.length} أقسام متوفرة`}</span>
        </div>
      </div>
      ...
      {!selectedSubCat ? (
        displayCategories.length === 0 ? (
          <div className="bg-white p-12 rounded-3xl border border-stone-200 text-center space-y-3">
            ...
            <h3 className="text-xl font-bold text-stone-700">لا توجد أقسام مسجلة في هذه الفئة حالياً</h3>
            <p className="text-stone-400 text-sm">يمكن للمدير إضافة أقسام جديدة وتحديد نوعها من لوحة التحكم.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {displayCategories.map((cat) => {
              const IconComponent = getCategoryIcon(cat.iconName, category);
              return (
                <div
                  key={cat.id}
                  onClick={() => setSelectedSubCat(cat.id)}
                  className="group bg-white p-6 rounded-2xl border border-stone-200/90 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between text-right cursor-pointer transform hover:-translate-y-1.5 relative overflow-hidden group-hover:shadow-emerald-500/20"
                >
                  <div className="absolute top-0 right-0 left-0 h-1.5 bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-700 opacity-80 group-hover:opacity-100 transition-opacity duration-300" />
                  
                  <div className="space-y-4 pt-1">
                    <div className="flex items-center justify-between">
                      <div className="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-700 p-0.5 shadow-md transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3">
                        <div className="w-full h-full bg-white rounded-[0.9rem] flex items-center justify-center transition-colors group-hover:bg-opacity-95">
                          <IconComponent className="w-8 h-8 text-emerald-600 transition-transform duration-300 group-hover:scale-110" />
                        </div>
                      </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-stone-500 text-sm leading-relaxed">
                        تصفح أحدث منتجات قسم {cat.name} الطازجة مباشرة من قاعدة البيانات
                      </p>
                    </div>
                  </div>
